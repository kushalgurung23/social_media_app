...
import 'package:spa_app/presentation/components/all/multiple_region_dropdown.dart';
...
class CenterRegisterProvider extends ChangeNotifier {
  GlobalKey<FormState> centerRegisterOneKey = GlobalKey<FormState>();
  GlobalKey<FormState> centerRegisterTwoKey = GlobalKey<FormState>();

  late TextEditingController centerNameTextController,
      userNameTextController,
      emailTextController,
      passwordTextController,
      confirmPasswordTextController;

  CenterRegisterProvider() {
    centerNameTextController = TextEditingController();
  ...
  String? validateCenterName(
      {required BuildContext context, required String value}) {
    if (value.trim().isEmpty) {
      return AppLocalizations.of(context).enterCenterName;
    } else if (value.length >= 100) {
      return AppLocalizations.of(context).centerNameTooLong;
    } else {
      return null;
    }
  }
  ...
  void showMultiSelect({required BuildContext context}) async {
    List<String>? teachingTypeResults = await showDialog(
      ...
      builder: (BuildContext context) {
        return MultipleRegionDropdown(
            titleHeading: AppLocalizations.of(context).selectRegion,
            firstRegionList: firstRegionList,
            secondRegionList: secondRegionList,
            thirdRegionList: thirdRegionList,
    ...
    if (teachingTypeResults != null) {
      selectedRegionItems = teachingTypeResults;
      selectedRegion = selectedRegionItems.join(", ");
      toggleRegionValidation(context: context);
      notifyListeners();
    }
  }

  void setCategory({required BuildContext context, required String newValue}) {
    categoryValue = newValue;
    toggleCategoryValidation(context: context, value: newValue);
    notifyListeners();
  }
  ...
  String? toggleCategoryValidation(
      {required BuildContext context, required String value}) {
    try {
      if (value == 'null') {
        categoryErrorMessage = AppLocalizations.of(context).selectCategory;
        return null;
      } else {
  ...
  void goBackFromScreenOne({required BuildContext context}) {
    toggleRegionValidation(context: context, prev: true);
    toggleCategoryValidation(context: context, value: 'prev');
    clearData();
    notifyListeners();
  ...
  void goToSecondScreen({required BuildContext context}) {
    if (regionErrorMessage == null && categoryErrorMessage == null) {
      Navigator.pushNamed(context, CenterRegisterScreenTwo.id);
    }
  }
  ...
  Future<void> registerCenter({required BuildContext context}) async {
    try {
      String? deviceToken = await FirebaseMessaging.instance.getToken();
      String body = jsonEncode({
        "center_name": centerNameTextController.text,
  ...
  void clearData() {
    selectedRegion = '';
    selectedRegionItems.clear();
    categoryValue = null;
    centerNameTextController.clear();
